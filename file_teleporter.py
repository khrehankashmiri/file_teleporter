...
class HistoryDialog(QDialog):
    ...
    def __init__(self, tab_name, history_entries, parent=None):
        super().__init__(parent)
        self.setWindowTitle(f"History - {tab_name}")
        self.resize(800, 600)  # Default size
        self.setMinimumSize(600, 400)  # Allow free resizing
...
class DropArea(QLabel):
    ...
    def __init__(self, main_window, tab_id, parent=None):
        super().__init__(parent)
    ...
    def has_valid_destination(self):
        return bool(self.dest_path) and os.path.isdir(self.dest_path)

    def update_style(self, highlight: bool = False):
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("File Teleporter - Professional File Management")
    ...
    def _apply_platform_styles(self):
    ...
    def _get_tab_config_by_id(self, tab_id):
        for t in self.config["tabs"]:
            if t["id"] == tab_id:
                return t
        return None

    def _update_tab_path_ui(self, tab_id, path):
    ...
    def _add_history_entry(self, tab_id, operation, src, dest, result):
    ...
    def set_operation(self, tab_id, operation):
        tab = self._get_tab_config_by_id(tab_id)
        if not tab:
            return
    ...
    def handle_drop(self, tab_id, src_paths):
        tab = self._get_tab_config_by_id(tab_id)
        if not tab:
    ...
    def show_about(self):
        QMessageBox.information(
            self,
            "About File Teleporter",
            (
                "📁 File Teleporter - Professional File Management\n\n"
                "A utility to teleport files and folders into\n"
